...
| Distributed tracing and external APM are absent. | Section 10 documents Prometheus metrics and Kubernetes monitoring, but no OpenTelemetry, Jaeger, Zipkin, Datadog, or New Relic tracing config exists. | Add OpenTelemetry instrumentation and trace propagation through BFF, Gateway, and backend service calls when deeper request correlation is needed. | 1-3 days |
